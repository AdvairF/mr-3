...
function Devedores({ devedores, setDevedores }) {
  ...
  const [sel, setSel] = useState(null);
  const [wp, setWp] = useState(null);
  const [abaParcelas, setAbaParcelas] = useState(false);
  const [form, setForm] = useState({
    nome:"", cpf_cnpj:"", tipo:"PJ", telefone:"", email:"",
    cidade:"Goiânia", status:"ativo",
    valor_original:"", data_vencimento:"",
    usar_parcelas: false, qtd_parcelas:"1", data_primeira_parcela:""
  });
  const F = (k,v) => setForm(f=>({...f,[k]:v}));
  ...
  function save() {
    const valor = parseFloat(form.valor_original) || 0;
    const parcelas = form.usar_parcelas && form.qtd_parcelas > 1 && form.data_primeira_parcela
      ? gerarParcelas(valor, parseInt(form.qtd_parcelas), form.data_primeira_parcela)
      : [{ id:1, numero:1, valor, vencimento: form.data_vencimento, status:"pendente", data_pagamento:null, observacao:"" }];
    const novo = { ...form, id: Date.now(), valor_original: valor, parcelas };
    setDevedores(p => [...p, novo]);
    setModal(false);
    setForm({ nome:"",cpf_cnpj:"",tipo:"PJ",telefone:"",email:"",cidade:"Goiânia",status:"ativo",valor_original:"",data_vencimento:"",usar_parcelas:false,qtd_parcelas:"1",data_primeira_parcela:"" });
  }

  function toggleParcela(devId, parcId, novoStatus) {
    setDevedores(prev => prev.map(d => {
      if (d.id !== devId) return d;
      const parcelas = (d.parcelas||[]).map(p => {
        if (p.id !== parcId) return p;
        return { ...p, status: novoStatus, data_pagamento: novoStatus === "pago" ? new Date().toISOString().slice(0,10) : null };
      });
      const todas = parcelas.length;
      const pagas = parcelas.filter(p=>p.status==="pago").length;
      const status = pagas === todas ? "pago" : pagas > 0 ? "negociando" : "ativo";
      return { ...d, parcelas, status };
    }));
    setSel(prev => {
      if (!prev || prev.id !== devId) return prev;
  ...
  function adicionarParcela(devId) {
    setDevedores(prev => prev.map(d => {
      if (d.id !== devId) return d;
      const parcelas = d.parcelas || [];
      const ultima = parcelas[parcelas.length - 1];
      const proxVenc = ultima ? (() => { const dd = new Date(ultima.vencimento+"T12:00:00"); dd.setMonth(dd.getMonth()+1); return dd.toISOString().slice(0,10); })() : new Date().toISOString().slice(0,10);
      const nova = { id: Date.now(), numero: parcelas.length+1, valor: ultima?.valor || 0, vencimento: proxVenc, status:"pendente", data_pagamento:null, observacao:"" };
      return { ...d, parcelas: [...parcelas, nova] };
    }));
    setSel(prev => {
      if (!prev || prev.id !== devId) return prev;
      const parcelas = prev.parcelas || [];
      ...
      const proxVenc = ultima ? (() => { const dd = new Date(ultima.vencimento+"T12:00:00"); dd.setMonth(dd.getMonth()+1); return dd.toISOString().slice(0,10); })() : new Date().toISOString().slice(0,10);
      const nova = { id: Date.now(), numero: parcelas.length+1, valor: ultima?.valor || 0, vencimento: proxVenc, status:"pendente", data_pagamento:null, observacao:"" };
      return { ...prev, parcelas: [...parcelas, nova] };
    });
  }

  function editarParcela(devId, parcId, campo, valor) {
    setDevedores(prev => prev.map(d => {
      if (d.id !== devId) return d;
      return { ...d, parcelas: (d.parcelas||[]).map(p => p.id !== parcId ? p : { ...p, [campo]: campo==="valor"?parseFloat(valor)||0:valor }) };
    }));
    setSel(prev => {
      if (!prev || prev.id !== devId) return prev;
      return { ...prev, parcelas: (prev.parcelas||[]).map(p => p.id !== parcId ? p : { ...p, [campo]: campo==="valor"?parseFloat(valor)||0:valor }) };
  ...
  function removerParcela(devId, parcId) {
    setDevedores(prev => prev.map(d => {
      if (d.id !== devId) return d;
      return { ...d, parcelas: (d.parcelas||[]).filter(p => p.id !== parcId) };
    }));
    setSel(prev => {
      if (!prev || prev.id !== devId) return prev;
      return { ...prev, parcelas: (prev.parcelas||[]).filter(p => p.id !== parcId) };
    });
  }
  ...
  const statusParcCor = { pago:"#16a34a", pendente:"#64748b", atrasado:"#dc2626", negociando:"#d97706" };
  const statusParclabel = { pago:"Pago", pendente:"Pendente", atrasado:"Atrasado", negociando:"Negociando" };
  ...
  return (
    <div>
      ...
      <div style={{ background:"#fff",borderRadius:18,border:"1px solid #f1f5f9",overflow:"hidden" }}>
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13 }}>
            <thead>
              <tr style={{ background:"#f8fafc" }}>
                {["Nome","CPF/CNPJ","Tipo","Status","Valor Total","Parcelas","WhatsApp",""].map(h=>(
                  <th key={h} style={{ textAlign:"left",padding:"10px 14px",color:"#64748b",fontWeight:700,fontSize:11,textTransform:"uppercase",letterSpacing:".05em" }}>{h}</th>
                ))}
              </tr>
            </thead>
                ...
                return (
                  <tr key={d.id} style={{ borderTop:"1px solid #f8fafc" }}>
                    ...
                    <td style={{ padding:"12px 14px" }}>
                      {parcelas.length > 0 ? (
                        <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                          <span style={{ fontSize:12,fontWeight:700,color:"#0f172a" }}>{pagas}/{parcelas.length}</span>
                          <div style={{ flex:1,minWidth:60,height:5,background:"#f1f5f9",borderRadius:99 }}>
                            <div style={{ height:5,width:`${parcelas.length?pagas/parcelas.length*100:0}%`,background:"#22c55e",borderRadius:99 }}/>
                          </div>
                          {atrasadas > 0 && <span style={{ fontSize:10,background:"#fee2e2",color:"#dc2626",padding:"1px 6px",borderRadius:99,fontWeight:700 }}>{atrasadas} atras.</span>}
                        </div>
                      ) : <span style={{ color:"#94a3b8",fontSize:12 }}>—</span>}
                    </td>
                    <td style={{ padding:"12px 14px" }}>
                      {d.telefone && <button onClick={()=>setWp(d)} style={{ background:"#dcfce7",color:"#15803d",border:"none",borderRadius:8,padding:"5px 10px",cursor:"pointer",display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700 }}>{I.wp} Enviar</button>}
                    </td>
                    <td style={{ padding:"12px 14px" }}>
                      <button onClick={()=>{ setSel(d); setAbaParcelas(false); }} style={{ color:"#94a3b8",background:"none",border:"none",cursor:"pointer" }}>{I.eye}</button>
                    </td>
                  </tr>
                );
              })}
      ...
      {/* Modal novo devedor */}
      {modal && (
        <Modal title="Novo Devedor" onClose={()=>setModal(false)} wide>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14 }}>
            <Inp label="Nome / Razão Social" value={form.nome} onChange={v=>F("nome",v)} span={2} />
            <Inp label="CPF / CNPJ" value={form.cpf_cnpj} onChange={v=>F("cpf_cnpj",v)} />
            <Inp label="Tipo" value={form.tipo} onChange={v=>F("tipo",v)} options={["PF","PJ"]} />
            ...
            <Inp label="Cidade" value={form.cidade} onChange={v=>F("cidade",v)} />
            <Inp label="Status" value={form.status} onChange={v=>F("status",v)} options={["ativo","negociando","pago","inativo"]} />
            <Inp label="Valor Total da Dívida (R$)" value={form.valor_original} onChange={v=>F("valor_original",v)} type="number" />
          </div>

          {/* Toggle parcelamento */}
          <div style={{ margin:"18px 0 14px",padding:16,background:"#f8fafc",borderRadius:12,border:"1.5px solid #e2e8f0" }}>
            <label style={{ display:"flex",alignItems:"center",gap:10,cursor:"pointer",marginBottom: form.usar_parcelas?14:0 }}>
              <input type="checkbox" checked={form.usar_parcelas} onChange={e=>F("usar_parcelas",e.target.checked)} style={{ width:16,height:16,accentColor:"#4f46e5",cursor:"pointer" }}/>
              <span style={{ fontWeight:700,fontSize:14,color:"#0f172a" }}>Parcelar a dívida</span>
              <span style={{ fontSize:12,color:"#94a3b8" }}>— gera parcelas automaticamente</span>
            ...
            {!form.usar_parcelas && (
              <div style={{ marginTop:10 }}>
                <Inp label="Data de Vencimento" value={form.data_vencimento} onChange={v=>F("data_vencimento",v)} type="date" />
              </div>
            )}
          </div>

          <div style={{ display:"flex",gap:10,marginTop:6 }}>
            <Btn onClick={save}>Salvar Devedor</Btn>
            <Btn onClick={()=>setModal(false)} outline>Cancelar</Btn>
          </div>
        </Modal>
      )}
      ...
      {sel && (
        <Modal title={sel.nome} onClose={()=>setSel(null)} wide>
          {/* abas */}
          <div style={{ display:"flex",gap:6,marginBottom:18,borderBottom:"2px solid #f1f5f9",paddingBottom:0 }}>
            {["Dados","Parcelas"].map((aba,i)=>(
              <button key={aba} onClick={()=>setAbaParcelas(i===1)}
                style={{ padding:"8px 18px",border:"none",background:"none",cursor:"pointer",fontFamily:"Mulish",fontWeight:700,fontSize:13,
                  color: (i===1)===abaParcelas?"#4f46e5":"#94a3b8",
                  borderBottom:`2px solid ${(i===1)===abaParcelas?"#4f46e5":"transparent"}`,marginBottom:-2 }}>
                {aba} {i===1 && sel.parcelas ? `(${sel.parcelas.length})` : ""}
              </button>
            ))}
          </div>
          ...
          {!abaParcelas && (
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,fontSize:13 }}>
              {[["Nome",sel.nome],["CPF/CNPJ",sel.cpf_cnpj],["Tipo",sel.tipo],["Cidade",sel.cidade],["Telefone",sel.telefone],["E-mail",sel.email]].map(([k,v])=>(
                <div key={k} style={{ padding:"10px 14px",background:"#f8fafc",borderRadius:10 }}>
                  <p style={{ fontSize:11,color:"#94a3b8",fontWeight:700,marginBottom:2 }}>{k}</p>
                  <p style={{ fontWeight:600,color:"#0f172a" }}>{v||"—"}</p>
                </div>
              ))}
              <div style={{ padding:"10px 14px",background:"#ede9fe",borderRadius:10 }}>
                <p style={{ fontSize:11,color:"#6d28d9",fontWeight:700,marginBottom:2 }}>Valor Total</p>
          ...
          {abaParcelas && (
            <div>
              {/* Resumo */}
              {(() => {
                const parcelas = sel.parcelas || [];
                const pagas = parcelas.filter(p=>p.status==="pago");
                const atrasadas = parcelas.filter(p=>p.status==="atrasado");
                const pendentes = parcelas.filter(p=>p.status==="pendente");
                const vlPago = pagas.reduce((s,p)=>s+p.valor,0);
                const vlPendente = [...atrasadas,...pendentes].reduce((s,p)=>s+p.valor,0);
                return (
                  <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16 }}>
                    {[
                      ["Total",parcelas.length,"#4f46e5","#ede9fe"],
                      ["Pagas",pagas.length,"#16a34a","#f0fdf4"],
                      ["Atrasadas",atrasadas.length,"#dc2626","#fef2f2"],
                      ["Pendentes",pendentes.length,"#d97706","#fefce8"],
                    ].map(([l,v,c,bg])=>(
                      <div key={l} style={{ background:bg,borderRadius:10,padding:"10px 12px",textAlign:"center" }}>
                        <p style={{ fontSize:10,color:c,fontWeight:700,marginBottom:3 }}>{l}</p>
                        <p style={{ fontFamily:"Syne",fontWeight:800,fontSize:20,color:c }}>{v}</p>
                      </div>
                    ))}
                  </div>
              ...
              {(() => {
                const parcelas = sel.parcelas || [];
                const pct = parcelas.length ? (parcelas.filter(p=>p.status==="pago").length/parcelas.length*100).toFixed(0) : 0;
                return (
                  <div style={{ marginBottom:16 }}>
                    <div style={{ display:"flex",justifyContent:"space-between",fontSize:12,color:"#64748b",marginBottom:5 }}>
                      <span>Progresso do pagamento</span><span style={{ fontWeight:700,color:"#16a34a" }}>{pct}%</span>
                    </div>
                    <div style={{ height:8,background:"#f1f5f9",borderRadius:99 }}>
                      <div style={{ height:8,width:`${pct}%`,background:"linear-gradient(90deg,#22c55e,#16a34a)",borderRadius:99,transition:"width .3s" }}/>
                    </div>
                  </div>
                );
              })()}
                ...
                <table style={{ width:"100%",borderCollapse:"collapse",fontSize:12 }}>
                  <thead>
                    <tr style={{ background:"#f8fafc",position:"sticky",top:0 }}>
                      {["Nº","Valor (R$)","Vencimento","Status","Pago em","Ação"].map(h=>(
                        <th key={h} style={{ padding:"8px 10px",textAlign:"left",color:"#64748b",fontWeight:700,fontSize:10,textTransform:"uppercase",letterSpacing:".04em" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {(sel.parcelas||[]).map((p,idx)=>{
                      const atrasada = p.status==="pendente" && new Date(p.vencimento+"T12:00:00") < new Date();
                      const statusReal = atrasada ? "atrasado" : p.status;
                      return (
                        <tr key={p.id} style={{ borderTop:"1px solid #f8fafc",background: statusReal==="atrasado"?"#fff9f9":statusReal==="pago"?"#f9fffe":"#fff" }}>
                          <td style={{ padding:"8px 10px",fontWeight:700,color:"#0f172a" }}>{p.numero || idx+1}</td>
                          <td style={{ padding:"8px 10px" }}>
                            <input type="number" value={p.valor} onChange={e=>editarParcela(sel.id,p.id,"valor",e.target.value)}
                          ...
                          <td style={{ padding:"8px 10px" }}>
                            <input type="date" value={p.vencimento} onChange={e=>editarParcela(sel.id,p.id,"vencimento",e.target.value)}
                              style={{ padding:"4px 7px",border:"1.5px solid #e2e8f0",borderRadius:6,fontSize:12,outline:"none" }}/>
                          </td>
                          <td style={{ padding:"8px 10px" }}>
                            <span style={{ fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:99,background: statusReal==="pago"?"#dcfce7":statusReal==="atrasado"?"#fee2e2":statusReal==="negociando"?"#fef9c3":"#f1f5f9",color:statusParcCor[statusReal]||"#64748b" }}>
                              {statusParclabel[statusReal]||statusReal}
                            </span>
                          </td>
                          <td style={{ padding:"8px 10px",color:"#64748b",fontSize:11 }}>{fmtDate(p.data_pagamento)}</td>
                          <td style={{ padding:"8px 10px" }}>
                            <div style={{ display:"flex",gap:4 }}>
                              {p.status!=="pago" && <button onClick={()=>toggleParcela(sel.id,p.id,"pago")} style={{ background:"#dcfce7",color:"#16a34a",border:"none",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:10,fontWeight:700 }}>✓ Pago</button>}
                              {p.status==="pago" && <button onClick={()=>toggleParcela(sel.id,p.id,"pendente")} style={{ background:"#f1f5f9",color:"#64748b",border:"none",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:10,fontWeight:700 }}>↩ Desfazer</button>}
                              {p.status==="pendente" && <button onClick={()=>toggleParcela(sel.id,p.id,"atrasado")} style={{ background:"#fee2e2",color:"#dc2626",border:"none",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:10,fontWeight:700 }}>! Atras.</button>}
                              <button onClick={()=>removerParcela(sel.id,p.id)} style={{ background:"#f8fafc",color:"#94a3b8",border:"none",borderRadius:6,padding:"3px 6px",cursor:"pointer",fontSize:11,fontWeight:700 }}>✕</button>
                            </div>
                          </td>
                        </tr>
                      );
              ...
              <Btn onClick={()=>adicionarParcela(sel.id)} outline>{I.plus} Adicionar Parcela</Btn>
            </div>
          )}
        </Modal>
      )}
