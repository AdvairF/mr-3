...
3. Utility:
   ...
   - Renderiza PDF A4 portrait com 7 seções:
     ...
     - **Resumo financeiro**: valor original + multa + honorários + correção INPC + juros + Art.523 + total atualizado + total pago + saldo devedor final
...
| **SC-3** | PDF seção identificação mostra credor (lookup `credores` via contrato.credor_id, com nome + CPF/CNPJ) + **TODOS** devedores com papéis (Phase 7.13 — PRINCIPAL + N coobrigados/avalistas/fiadores/cônjuge/outros) + responsabilidade + CPF/CNPJ por devedor | Validar com contrato multi-devedor real (M L FRIOS UUID `335a2ad2-...` ou similar). **Se operador não tiver contrato multi-devedor disponível**: criar contrato cobaia em prod com 1 PRINCIPAL + 1 COOBRIGADO advair (devedor de teste id=8) pra exercitar pattern. **Após SC-3 PASS, deletar contrato cobaia** (não polui prod) |
