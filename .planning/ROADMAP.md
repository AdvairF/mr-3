...
### Phase 7.10.bug2: Custas avulsas (`_so_custas:true`) UI bugs — DOIS sub-bugs (Sub-bug 1 INSERTED 2026-05-01, EXPANDIDA 2026-04-26, REVISADA 2026-04-28, sub-bug 2 BACKLOG)
...
**Status:** Sub-bug 1 **INSERTED** 2026-05-01 — Helper INPC para `_so_custas` (UI "Calculando..." indefinido). CONTEXT.md commitado em `ee1953f` no pai. 169 linhas, 7 D-pre lockeds (D-pre-1 helper signature, D-pre-2/3 branches em 2 callsites, D-pre-4 D-01 motor INTOCADO, D-pre-5 NÃO filtra c.pago via precedente Phase 7.9 P3, D-pre-6 c.data legacy shape, D-pre-7 test parity helper === motor), 1 plan único / 5 tasks / ~25-30 linhas, severidade BAIXA, autonomous:false. Decisão arquitetural Caminho A locked: helper externo `utils/custasAvulsas.js` espelha motor `devedorCalc.js:479-491`; UI consumers (TabelaDividas L94-96 + DetalheDivida L79-86) ganham branch `_so_custas` antes do fallback null. TASK 0 operador 2026-05-01 confirmou frequência alta (~50% linhas módulo Dívidas afetadas, todas com Valor Original R$ 0,00). 5 memory feedbacks aplicados (incluindo `memory_must_factcheck_codebase_before_claiming_severity` — catch pré-commit do Claude Code evitou referência fantasma a `feedback_validate_phase_premise_with_user_use_case_first` planejada na Phase 7.13d mas nunca criada). Pendente: `/gsd-plan-phase 7.10.bug2.sub1` em sessão futura (pattern saudável 1 phase = 1 sessão). Ver `.planning/phases/07.10-bug2-sub1-custas-helper-inpc/07.10-bug2-sub1-CONTEXT.md`. Sub-bug 2 (schema dual `pago_advogado`/`quitado_devedor`) permanece **BACKLOG** — defer pra phase própria, sem dependência sub-bug 1. Original: Backlog 2026-04-25 (sub-bug 1) + EXPANDIDA 2026-04-26 com sub-bug 2 + **REVISADA 2026-04-28** (severidade rebaixada ALTA → MÉDIA-BAIXA após audit técnico que confirmou motor cobra correção INPC corretamente — bug é apenas VISUAL/semântico em UI badge). Sem prioridade fixa. 7.13c (encargos arquitetural, ALTA verdadeira) prioritária acima desta. Phase 7.14 SHIPPED 2026-04-28 — não bloqueia mais.
