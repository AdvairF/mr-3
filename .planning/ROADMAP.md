...
**Status:** **EM EXECUÇÃO Task 6** 2026-04-30 — Tasks 1-5 completas (commits `45df63b` CONTEXT + `c64bc98` PLAN + submódulo `b6b48d9` feat bugfix excluirCusta cascade + anti-zero validation server+client + pai `64dd7ee` chore bump — todos pushados 2026-04-30 pós-PAUSA #1 Vercel Ready). Task 6 sub-passo 6.b ABORT GATE rodado: **escopo reduzido** — Sub-cleanup B (3 advair) **REMOVIDO** desta phase. Investigação Q14 + dados granulares revelou que as 3 advair têm `qtd_custas=1` cada (custas reais "honorarios judicial" R$ 150) — NÃO são órfãs vazias do mecanismo Q2 conservative. Operador (advogado, dono) confirmou são UAT exploratório próprio. Defer pra **Phase 7.10.bug5 BACKLOG** (cleanup devedor de teste advair id=8 — escopo distinto). Phase passa a entregar apenas: Sub-cleanup A (7 órfãs MENDES) + bugfix excluirCusta cascade + anti-zero validation. 4 SCs UAT (SC2 N/A removido). 6 D-pre lockeds inalterados (D-pre-1 mecânica preservada — definição de "cleanup ESPECÍFICO" abriga critério genérico Sub-A). PLAN.md ajustado in-place 2026-04-30 (commit pendente junto com push final pós-UAT). Backlog inserido: Phase 7.10.bug5 (advair test devedor cleanup, severidade BAIXA, escopo separado). Backlog pré-existente Phase 7.10.bug4 mantido (FK CASCADE Migration 007).
