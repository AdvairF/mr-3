...
### Phase 7.10.bug2: Custas avulsas (`_so_custas:true`) UI bugs — DOIS sub-bugs (Sub-bug 1 SHIPPED 2026-04-30, EXPANDIDA 2026-04-26, REVISADA 2026-04-28, sub-bug 2 BACKLOG)
...
**Status:** Sub-bug 1 **SHIPPED** 2026-04-30 — Tag `v1.4-phase7.10.bug2.sub1` em `61dbabe` (pai chore bump expansion). Helper INPC `src/utils/custasAvulsas.js` retorna `{ original, atualizado }` espelhando shape motor `result.custas` (devedorCalc.js:513). Plan original (commit `9c3da4d`): helper signature `→ number` + branches `_so_custas` em TabelaDividas L94-96 + DetalheDivida L79-86. **Expansion mid-UAT 2026-04-30** (commit `21fd3b4`): operador requisitou também exibir Valor Original (soma raw c.valor) — pattern `ux_consistency_principle_drives_scope_expansion` (precedente Phase 7.14b, segunda aplicação confirma pattern). Refactor Opção A: helper signature `→ { original, atualizado }`. 5 commits cumulative (2 submódulo + 3 pai + 1 docs CONTEXT preservado de `ee1953f`). UAT prod 5+1/5+1 SCs PASS REAL: SC1 Saldo Atualizado em TabelaDividas + SC1.5 NOVO Valor Original soma raw + SC2 DetalheDivida consistente + SC3 console limpo + SC4 dívidas regulares preservadas + SC5 cross-check parity X1≡X2≡X3 motor `result.custas.{original,atualizado}`. 7 D-pre lockeds validados (D-pre-1 ajustado mid-UAT scalar→object). 8/8 helper tests PASS (Test 8 NOVO `.original` sem fator INPC + Test 7 expandido para parity dual `.original`+`.atualizado` ±1e-9). 34/34 motor regression PASS cumulative. D-01 motor INTOCADO cumulative dual gate confirmed (4a working tree + 4b post-commit) em ambos feat commits. Pre-flight: gsd-plan-checker YELLOW 1 BLOCKER fixado (typo `calcularDetalhesAtualizados` → `calcularDetalheEncargos` em 5 linhas PLAN, factchecked via grep direto antes EDITs — lição `memory_must_factcheck_codebase_before_claiming_severity`). 7 memory feedbacks aplicados (so_custas + custas_semantica + post_hoc_d_pre + discuss_walkthrough + memory_must_factcheck + gate_motor_grep_name_only + state_pattern_paralelo). 2 memory feedbacks NOVOS candidatos pendentes consolidação: `ux_consistency_principle_drives_scope_expansion` REFORÇO (segunda aplicação após 7.14b candidato) + `helper_signature_mirror_motor_return_shape` NOVO (helper espelhando motor shape evita refactor mid-UAT scalar→object). Ver `.planning/phases/07.10-bug2-sub1-custas-helper-inpc/07.10-bug2-sub1-01-SUMMARY.md`. Sub-bug 2 (schema dual `pago_advogado`/`quitado_devedor`) permanece **BACKLOG** — defer pra phase própria, sem dependência sub-bug 1. Original: Backlog 2026-04-25 (sub-bug 1) + EXPANDIDA 2026-04-26 com sub-bug 2 + **REVISADA 2026-04-28** (severidade rebaixada ALTA → MÉDIA-BAIXA após audit técnico que confirmou motor cobra correção INPC corretamente — bug é apenas VISUAL/semântico em UI badge). Sem prioridade fixa. 7.13c (encargos arquitetural, ALTA verdadeira) prioritária acima desta. Phase 7.14 SHIPPED 2026-04-28 — não bloqueia mais.

### Phase 7.10.bug3: Limpeza Custas Órfãs + Bugfix excluirCusta + Anti-Zero Validation (INSERTED 2026-04-30)

**Goal:** Limpar 10 dívidas-fantasma órfãs em prod (7 `_so_custas:true` com `custas:[]` vazio do MENDES E MENDES devedor_id=25 contrato `ce7b8d47` + 3 lixo de teste advair id=8) + corrigir mecânica raiz que gera órfãs (Q2 conservative Phase 7.9 em `excluirCusta`) + adicionar validação anti-zero em `criarCusta`. Cleanup é específico (não regra genérica `WHERE contrato_id IS NULL` — risco false positive em legacy data).

**Depends on:** Phase 7.10.bug2.sub1 SHIPPED (helper retorna `{original:0, atualizado:0}` para array vazio — Test 6 garante que cleanup não quebra UI durante deploy) + Phase 7.9 D-23 (`criarCusta` + `excluirCusta` service layer).

**Requirements:** (limpeza dado órfão visível diariamente; defesa-em-profundidade contra futura sujeira pelo mesmo mecanismo; bloqueio anti-zero porque custa zero não é cadastro de escritório — isenção é decisão judicial).

**Decisions:** ver `.planning/phases/07.10-bug3-limpeza-custas-orfas-anti-zero/07.10-bug3-CONTEXT.md` (252 linhas, 6 D-pre lockeds, mecânica Q2 conservative confirmada via investigação SQL queries 1-14, ABORT GATE pre-cleanup, Parte 4 FK CASCADE Migration 007 DEFERRED Phase 7.10.bug4 BACKLOG).

**Plans:** 1 (único, 6 tasks: investigation grep + bugfix excluirCusta + anti-zero server + anti-zero client + gates+commit + sql ops UAT).

**UI hint:** yes (form custa avulsa identificado via Task 1 grep audit pós-7.14b removeu CustasAvulsasForm).

**Status:** **SHIPPED** 2026-05-01 — Tag `v1.4-phase7.10.bug3` em `31116ba` (pai chore bump fix-forward dbDelete signature). 7 órfãs MENDES limpas via SQL DELETE Sub-cleanup A (total dividas 150→143). Bugfix `excluirCusta` cascade DELETE ativo em prod. Anti-zero validation tripla funcional (defesa em profundidade). UAT prod 4/4 SCs efetivos PASS (SC1+SC3+SC4+SC5; SC2 N/A removido pós-ABORT GATE 6.b). **Scope reduction mid-execução** (commit pai `352366f`): 3 advair NÃO eram órfãs vazias (qtd_custas=1, custas reais "honorarios judicial" R$ 150) — defer Phase 7.10.bug5 BACKLOG. **Fix-forward dbDelete signature** (submódulo `80f8f1c` + chore bump pai `31116ba` TAG TARGET): UAT SC3 detectou bug crítico — `dbDelete` espera UUID puro, não query string completa (espelha L393 `excluirContrato` + L471 `excluirDocumento` preexistentes prod cumulative). Bug NÃO foi pego por build + test:regressao 34/34 + Gate D-01 dual + lint (sintaxe válida JavaScript, semântica errada PostgREST). Lição cumulative: UAT humano em prod é detector primário pra semantic-drift de helpers compartilhados. **Commits cumulative pai (5):** `45543d5` CONTEXT + `c64bc98` PLAN + `64dd7ee` chore bump (com bug Task 2) + `352366f` docs scope reduction + `31116ba` chore bump fix-forward (TAG TARGET). **Commits submódulo (2):** `b6b48d9` feat (com bug) + `80f8f1c` fix dbDelete signature. D-01 motor INTOCADO cumulative dual gate confirmed em ambos commits submódulo. Schema INTOCADO (Migration 007 FK CASCADE permanece DEFERRED Phase 7.10.bug4 BACKLOG). **String drift descoberta SC4:** 3 validadores client (form preexistente "Informe um valor maior que zero." + Task 4 + Task 3 "Valor da custa deve ser maior que zero") com mensagens divergentes — defesa em profundidade tripla funcional (zero falha funcional, robustez de bônus), unificação de string defer Phase 7.10.bug6 BACKLOG. 8 memory feedbacks aplicados + 4 NOVOS candidatos: `helpers_with_internal_prefix_require_callsite_audit` NOVO + `preexisting_form_validators_must_be_audited_in_anti_X_phases` NOVO + `abort_gate_during_execution_can_reduce_scope` NOVO + `ux_consistency_principle_drives_scope_expansion` REFORÇO **terceira aplicação**. SUMMARY (`07.10-bug3-01-SUMMARY.md`) + LEARNINGS (`07.10-bug3-LEARNINGS.md`) gerados. 19 tags v1.4 ativas (era 18). 2 deviations registradas: D-1 scope reduction Sub-cleanup B (NÃO regressão — gate funcionando) + D-2 fix-forward dbDelete 1 linha. **Backlog inserido (2 phases NEW):** Phase 7.10.bug5 (BAIXA — cleanup 3 advair test devedor) + Phase 7.10.bug6 (BAIXA — unificar string anti-zero entre 3 validadores).

### Phase 7.10.bug4: FK CASCADE Migration 007 (BACKLOG 2026-05-01)
**Severidade:** **BAIXA** — defesa preventiva. Junction devedores_dividas (Migration 002:161) já tem ON DELETE CASCADE preservada e operacional. Phase 7.10.bug4 visa estender o pattern pra outras FKs em dividas/custas/pagamentos/etc que ainda dependem de cleanup manual ou cascateamento aplicacional.
**Goal:** Aplicar Migration 007 com FK CASCADE em relações ainda não cobertas. Mapear FKs faltantes via SELECT pg_get_constraintdef em information_schema, identificar candidatas (FKs sem ON DELETE rule explícita ou com ON DELETE NO ACTION), aplicar ON DELETE CASCADE seletivamente onde semantica de "filho não sobrevive sem pai" se aplica.
**Trigger:** Phase 7.10.bug3 D-pre-4 referenciou como "DEFERRED — defesa preventiva, baixa prioridade". Pattern de Migration 002:161 (junction devedores_dividas CASCADE) provou-se útil em Phase 7.10.bug3 SC3 (cascade DELETE de dívida-fantasma propagou cleanup automático). Generalizar.
**Depends on:** nenhum. Phase stand-alone.
**Requirements:** Migration nova (007) ALTER TABLE ... DROP CONSTRAINT + ADD CONSTRAINT ... ON DELETE CASCADE. Backup snapshot pré-migration obrigatório (defesa em profundidade — migrations destrutivas em FK podem cascatear deleções não-desejadas se mal-aplicadas).
**Decisions:** TBD em CONTEXT.md futura. Pendentes:
- D-pre-1: Quais FKs entram (lista completa de candidatas via audit information_schema)
- D-pre-2: Backup table snapshot pré-migration
- D-pre-3: ABORT GATE pré-aplicação obrigatório (count tabelas + FKs)
- D-pre-4: Smoke test pós-migration (cascade real exercitado)
- D-pre-5: D-01 motor INTOCADO (Migration é schema, motor não toca schema)
**Plans:** 1 plan estimado (~15-30 min execução): audit FKs + backup + Migration 007 + smoke test + UAT.
**UI hint:** no (Migration backend apenas; UI não muda).
**Status:** **BACKLOG** 2026-05-01 — defer. Severidade BAIXA, sem urgência. Phase 7.10.bug2 sub-bug 2 + Phase 8 PDF + Phase 7.10.bug5 + Phase 7.10.bug6 + Phase 7.13d.bug2 têm prioridade acima desta.

### Phase 7.10.bug5: Cleanup 3 dívidas advair (devedor de teste id=8) (BACKLOG 2026-04-30)
**Severidade:** **BAIXA** — sujeira visual em devedor de teste do próprio operador. NÃO afeta cobrança real. Operador raramente filtra por "advair" no Módulo Dívidas em uso normal.
**Goal:** Limpar 3 dívidas legacy advair id=8 com `_so_custas:true` e `qtd_custas=1` cada (custa "honorarios judicial" / "Honorários judiciais" R$ 150,00, data 2026-02-01/02, `pago=NULL`). IDs: `85066e3d-289d-4cd2-9b00-09e8f219a5cd`, `d82841e3-33d3-4c36-8a8d-93c7398530a1`, `6cd6e321-62cd-4f68-b3aa-65d0f7a4f00f`. Mecânica de criação: UAT exploratório do próprio operador imitando dado real, não cobrança real.
**Trigger:** Phase 7.10.bug3 ABORT GATE 2026-04-30 sub-passo 6.b detectou divergência vs investigação 30-abr (esperado: advair em `regulares_sem_contrato`; real: advair em `so_custas_com_dados` com qtd=1). Decisão de escopo do operador: separar cleanup mecânico (7.10.bug3) de cleanup test devedor (7.10.bug5 esta).
**Depends on:** nenhum. Pode rodar isolado quando o operador decidir.
**Requirements:** (cleanup específico de 3 IDs lista fechada; sem mecânica programática; sem audit trail necessário — devedor de teste do próprio dono)
**Decisions:** SQL DELETE direto via lista de 3 IDs no Supabase SQL Editor + smoke check filtro "advair" zero linhas R$ 150 órfãs. Sem bugfix incluso (mecânica raiz Q2 conservative já tratada na 7.10.bug3). Sem migration. Sem mudança de código.
**Plans:** 1 plan trivial (~5 min execução): pre-check Query B preview (3 rows existem) → BACKUP snapshot opcional → DELETE 3 → smoke filtro UAT → opcional drop backup.
**UI hint:** no (operação SQL ops apenas; UI valida pós-execução).
**Status:** **BACKLOG** 2026-04-30 — defer. Próxima sessão pode priorizar dependendo de severidade comparada. Severidade BAIXA, sem urgência. Phase 7.10.bug2 sub-bug 2 (custas semantics, ALTA — perda receita) + Phase 7.13d.bug2 (DetalheContrato edit anomalia) + Phase 7.10.bug4 (FK CASCADE Migration 007) têm prioridade acima desta.

### Phase 7.10.bug6: Unificar string anti-zero entre 3 validadores client+server (BACKLOG 2026-05-01)
**Severidade:** **BAIXA** — string drift cosmético entre 3 validadores. ZERO falha funcional (defesa em profundidade tripla bloqueia anti-zero corretamente). Operador entende ambas mensagens (português coerente em ambas).
**Goal:** Unificar mensagem anti-zero entre 3 camadas que cobrem a mesma propriedade `valor > 0` em criação de custa:
1. **Client preexistente** (form custa avulsa, Phase 7.9 ou anterior): `"Informe um valor maior que zero."` ← dispara primeiro
2. **Client Task 4** (handler `handleCriarCusta` em DetalheContrato.jsx, Phase 7.10.bug3): `"Valor da custa deve ser maior que zero"` ← nunca dispara
3. **Server Task 3** (`criarCusta` em services/contratos.js, Phase 7.10.bug3): `"Valor da custa deve ser maior que zero"` ← nunca dispara

**Decisão arquitetural pendente:** 2 caminhos viáveis:
- **Opção A** — unificar todas em 1 mensagem (escolher uma das duas + propagar pra outras 2)
- **Opção B** — remover validador preexistente, novo guard Task 4 vira primário (fallback Task 3 server preserva defesa em profundidade dupla)

**Trigger:** Phase 7.10.bug3 SC4 UAT 2026-05-01 detectou string drift. Plan original 7.10.bug3 D-pre-3 prometia "mensagem unificada" mas assumia 2 validadores (Task 3 server + Task 4 client). Validador preexistente NÃO foi auditado pré-execute → fica fora do plan. Memory feedback NOVO: `preexisting_form_validators_must_be_audited_in_anti_X_phases`.
**Depends on:** nenhum. Phase trivial, ~5-10 linhas de mudança em 1-3 arquivos.
**Requirements:** (consistência cosmética entre validadores; zero quebra funcional; preferência por Opção A se plan checker considerar manutenção fácil)
**Decisions:** TBD — operador decide A vs B em discuss-phase.
**Plans:** 1 plan trivial (~5-10 min): grep callsites client preexistente + decisão A/B + EDIT 1-3 arquivos + UAT 1 SC re-trigger anti-zero validate string única.
**UI hint:** yes (form custa avulsa identificado em Phase 7.10.bug3 Task 1 — provavelmente NovaCustaJudicial.jsx ou similar).
**Status:** **BACKLOG** 2026-05-01 — defer. Severidade BAIXA, zero urgência. Phase 7.10.bug2 sub-bug 2 + Phase 7.13d.bug2 + Phase 7.10.bug4 + Phase 7.10.bug5 têm prioridade acima desta.
...
### Phase 7.14: Blindagens de Integridade de Dados (SHIPPED 2026-04-28)
...
**Status**: **SHIPPED** 2026-04-28 — tag `v1.4-phase7.14` em `275af36` (pai chore bump submódulo). 4 plans atomic bisect-able executados (01 soft delete devedor + Migration 003 deleted_at TIMESTAMPTZ + ProcessosJudiciais badge "Inativo" forense / 02 DELETE NovaDivida.jsx + remove botão obsoleto / 03 criarContrato UI guard + service guard / 04 parseInputBR.js + InputBR wrapper + Strategy A delegation 31 inputs). Final Gates 10/10 GREEN. 5 memory feedbacks NOVOS aplicados. SUMMARY 01-04 + LEARNINGS commitados (`ee26490`).
...
### Phase 7.14b: Remover forms legacy do drawer Pessoas/Devedores (SHIPPED 2026-04-30)
...
**Status**: **SHIPPED** 2026-04-30 — Tag `v1.4-phase7.14b` em `155c48d` pai (chore bump). Plan original entregue: <DividaForm> legacy + DividaForm.jsx (167 linhas) deletados do drawer Pessoas/Devedores. **Expansion durante UAT**: <CustasAvulsasForm> também removido (princípio "tudo dentro do contrato" declarado pelo orchestrator coerente com Phase 7.14 D-pre-6). 2 commits submódulo (`f0cc63e` feat remove DividaForm + 24 linhas App.jsx + DividaForm.jsx delete; `45b65a8` feat.expand remove CustasAvulsasForm callsite -3 linhas) + 2 commits pai (`ad21dbb` bump f0cc63e + `155c48d` bump 45b65a8 = TAG TARGET). Component CustasAvulsasForm L2465-2511 + handler adicionarCustasAvulsas L3311-3357 (~94 linhas) preservados dead — minimal cleanup, tree-shake remove do bundle. Bundle cumulative reduction ~10KB+. UAT prod 4/4 SCs PASS REAL: SC1 ambos forms ausentes drawer + SC2 path alternativo via DetalheContrato → +Adicionar Documento confirmado + SC3 console limpo + SC4 regressão zero outras telas. Bug visual pré-existente Phase 7.13 cumulative ("custas reportam success mas não aparecem na lista") resolvido implicitamente. D-01 motor INTOCADO cumulative dual gate confirmed (devedorCalc + services/pagamentos + services/dividas + masks zero modifications). Custas continuam disponíveis via fluxo Phase 7.9 _so_custas:true. Memory feedback NOVO candidato: `ux_consistency_principle_drives_scope_expansion` (princípio arquitetural declarado pelo operador é gatilho válido pra expansion durante UAT). Dependência ROADMAP "1-2 dias de uso real pós-7.14 SHIP" cumprida (3 dias). Ver `.planning/phases/07.14b-drawer-pessoas-cleanup/07.14b-01-SUMMARY.md`.

### Phase 7.13d: Encargos Defaults Vazios — NovoContrato wizard (SHIPPED parcial 2026-04-30)
...
**Status:** **SHIPPED parcial** 2026-04-30 — Tag `v1.4-phase7.13d` em `f80a8e3` pai (chore bump). Path E entregue: NovoContrato wizard SC1+SC2+SC3 PASS REAL prod (contrato `f4417baa` criado: multa=30, juros_am=1.5, honor=0, juros_tipo="outros", indexador=inpc) + AdicionarDocumento fallbacks alinhados + Inp.jsx honra `o.disabled` (bw-compat short-circuit) + correcao.js sentinel "— Selecione —" disabled em INDICE_OPTIONS + JUROS_OPTIONS. SC4a PASS (DetalheContrato sem mexer preserva). SC4b FAIL — anomalia "input N → DB N-x" não-determinística (15→12, 25→24) descoberta em UAT, INDEPENDENTE de Path E (pipeline cliente lossless verificado, NovoContrato funciona com mesmo pipeline). SC4c+SC5 NÃO TESTADOS por fadiga UAT (mesma classe SC2/SC3 que PASS). Submódulo: 2 commits atomic (`58cc6fa` feat 4 files + `2071c46` fix-forward DetalheContrato — smoking gun PRIMARY: `initEditForm` helper standalone L75-91 com OLD DEFAULTS escapou da varredura grep do plan original). Pai: 4 commits (`3fae579` insert + `1734568` plan + `9aead9b` bump 58cc6fa + `f80a8e3` bump 2071c46 = TAG TARGET). Bug SC4b deferred to **Phase 7.13d.bug2 BACKLOG** (BAIXA-MÉDIA — operador raramente edita contratos existentes; motor lê das DIVIDAS via junction → cálculos jurídicos preservados mesmo com header corrompido). DB ACORDO `ce7b8d47` restaurado SQL UPDATE pós-corrupção UAT (multa=10 + honor=10 cumulative). D-01 motor INTOCADO cumulative confirmed dual gate (4a working tree + 4b post-commit zero matches em devedorCalc.js + services/pagamentos.js + services/dividas.js + masks.js). Reconciliações documentadas: `npm run lint` script nunca configurado (skip); `npm test` (full) tem `filaDevedor.test.js` empty stub pre-existente Phase 7.13b (skip); gate efetivo é `npm run test:regressao` 8 files / 34 tests motor regression (auto-roda via `prebuild` hook). 7 memory feedbacks aplicados (5 originais + 2 NOVOS): `grep_state_pattern_paralelo_em_componentes_relacionados` (lição smoking gun) + `independent_bug_during_uat_does_not_block_phase` (lição shipped parcial). Ver `.planning/phases/07.13d-encargos-defaults-vazios/07.13d-01-SUMMARY.md` + `.planning/phases/07.13d-encargos-defaults-vazios/07.13d-LEARNINGS.md`.

### Phase 7.13d.bug2: DetalheContrato edit anomalia — input N → DB N-x (BACKLOG 2026-04-30)
**Severidade:** BAIXA-MÉDIA — operador raramente edita contratos existentes; motor preserva cálculos jurídicos via dividas (junction). Header `contratos_dividas` corrompido tem impacto declarativo apenas. Bug não-determinístico, mitigação prática: "salvar 2x preserva valor" (workaround reportado pelo orchestrator).
**Goal:** Identificar causa raiz de anomalia "input N → DB N-x" em DetalheContrato edit flow. Pipeline cliente lossless já verificado (Inp → InputBR → state → parseFloat → dbUpdate REST PATCH puro). NovoContrato com mesmo pipeline funciona em SC3 PASS. Suspeitas: bug pré-existente latente, browser quirk (locale, autofill, extension), pg_trigger Supabase. Investigação: Network tab capture (UI vs payload vs DB triangulação) + pg_trigger audit + reprodução em browser anônimo/outro browser.
**Trigger:** Descoberta em UAT 7.13d 2026-04-30 SC4b. Inicialmente pareceu regressão de Path E (DetalheContrato fix-forward `2071c46` mexeu em initEditForm); investigação read-only do pipeline cliente revelou lossless completo, NovoContrato (mesmo pipeline) funciona corretamente. Bug é INDEPENDENTE de Path E.
**Depends on:** nenhuma — phase independente. Pré-requisito empírico: Network tab capture pelo orchestrator durante save (não foi feito por fadiga UAT).
**Plans:** TBD — investigação primeiro, depois plan.
**Status:** **BACKLOG** 2026-04-30 — defer. Próxima sessão pode priorizar dependendo de severidade comparada. Phase 7.10.bug2 (sub-bug 2 custas, ALTA — perda receita) tem prioridade acima desta.
