...
**Status:** **INSERTED** 2026-04-30 — CONTEXT.md commitado em pai. 6 D-pre lockeds: D-pre-1 cleanup ESPECÍFICO (Sub-A genérico `_so_custas=true AND jsonb_array_length=0 AND devedor_id=25` + Sub-B lista 3 UUIDs advair), D-pre-2 bugfix `excluirCusta` cascade DELETE quando array vira vazio E `_so_custas=true` (junction devedores_dividas CASCADE Migration 002:161), D-pre-3 anti-zero `valor > 0` server+client (mensagem unificada), D-pre-4 D-01 motor INTOCADO, D-pre-5 pre-cleanup COUNT global obrigatório (Query 7+14+AJUSTE 1) com ABORT GATE, D-pre-6 cleanup escopado anti-blast-radius. Severidade MÉDIA (sujeira visual; defesa-em-profundidade existe via 7.10.bug2.sub1 helper). Risco BAIXO. autonomous:false. 5 UAT scenarios (SC1 MENDES 13 linhas, SC2 advair zero órfãs, SC3 bugfix `excluirCusta` cascade, SC4 anti-zero submit bloqueado, SC5 cleanup global Query 14 zerada). 8 memory feedbacks aplicáveis incluindo `feedback_blindagens_integridade_apos_funcionamento` (cadência 3-4 features → 1 blindagem; Phase 7.9 + 7.10bcd + 7.10.bug2.sub1 = 3 features) + `feedback_defensive_grep_when_audit_undercounts` (investigação ampliada queries 7-14 pegou 3 órfãs advair que escaparam diagnóstico inicial focado em MENDES). Pendente: `/gsd-plan-phase 7.10.bug3` em sessão futura. Backlog inserido: Phase 7.10.bug4 (FK CASCADE Migration 007, severidade BAIXA, defesa preventiva).
