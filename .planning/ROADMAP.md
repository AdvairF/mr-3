...
### Phase 7.13c: Encargos — Contrato como Fonte Única (INSERTED)
**Severidade:** ALTA — bug sistêmico financeiro descoberto em UAT 7.13b PAUSA #1-RETOMADA. Saldo "atualizado" exibido em prod desde Phase 7.8 usa encargos das dívidas individuais (drift sistêmico de `contratos_dividas` em 3 contratos / 147 dívidas — audit prod 2026-04-29). Afeta 5 consumers: DetalheContrato + Pessoas/Detalhe-Devedor + Carteira Total Dashboard + FilaPainel/FilaPesquisa.

⚠ **DECISÕES FINANCEIRAS RETROATIVAS:** Saldos exibidos pelo sistema desde Phase 7.8 (quando `calcularDetalheEncargosContrato` foi introduzido) podem estar incorretos para 3 contratos com drift / 147 dívidas afetadas (audit prod 2026-04-29 Q1). Recomenda-se auditar relatórios e decisões financeiras tomadas com base em valores exibidos pelo sistema antes do shipping da 7.13c. **D-pre-7.3 ABORT GATE** (em CONTEXT.md): se delta no comparison report Plan 02 surpreender o orchestrator em qualquer dos 3 contratos, deploy ABORTA e phase volta a discuss revisado.
...
**Status:** **INSERTED** 2026-04-29 — CONTEXT.md (366 linhas) + AUDIT-PROD.md (133 linhas) commitados em `03e6b65` no pai (commit `9e727c4` reset --soft revisitado em PAUSA #1, refinamento substantivo via 8 edits, novo commit `03e6b65`). Path A puro locked após Q1+Q2 audit prod: drift sistêmico mensurável (3 contratos / 147 dívidas) + zero heterogeneidade intra-contrato. Pendente: `/gsd-plan-phase 7.13c`. **Severidade ALTA** — bloqueia confiabilidade financeira do app + DECISÕES FINANCEIRAS RETROATIVAS warning ativo até 7.13c SHIPPED.
